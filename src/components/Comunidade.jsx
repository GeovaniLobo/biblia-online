...
export default function Comunidade({ usuarioLogado, darkMode, onVerPerfil }) {
  ...
  const [usuarioStoryVisualizando, setUsuarioStoryVisualizando] =
    useState(null);
  const [indiceStoryAtual, setIndiceStoryAtual] = useState(0);
  const [progressoStory, setProgressoStory] = useState(0);
  const [modalVisualizadoresAberto, setModalVisualizadoresAberto] =
    useState(false);
  ...
  const [menuSugestoesMencaoAberto, setMenuSugestoesMencaoAberto] =
    useState(false);

  const [novoComentario, setNovoComentario] = useState({});
  const [respondendoComentarioId, setRespondendoComentarioId] = useState({});
  const [menuMencaoComentarioAberto, setMenuMencaoComentarioAberto] =
    useState(null);
  const [termoBuscaMencaoComentario, setTermoBuscaMencaoComentario] =
    useState("");
  ...
  useEffect(() => {
    ...
    async function carregarDadosIniciais() {
      try {
        ...
        const notifs = await BancoDeDados.getNotificacoes(
  ...
  useEffect(() => {
    async function carregarMensagens() {
      if (chatComUsuario) {
        const msgs =
          typeof BancoDeDados.getMensagensChat === "function"
            ? await BancoDeDados.getMensagensChat(
                usuarioLogado.username,
                chatComUsuario,
              )
            : [];
  ...
  const meuPerfilBanco =
    perfisReais.find((p) => p.username === usuarioLogado.username) || {};
  const fotoPerfilOficial =
    meuPerfilBanco.foto ||
    usuarioLogado.foto ||
    "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80";
  const nomePerfilOficial =
    meuPerfilBanco.nome || usuarioLogado.nome || "Usuário";
  ...
  const storiesFiltradosAmigos = storiesValidos.filter((s) =>
    amigosMaisLogado.includes(s.username),
  );

  const meusStories = storiesValidos.filter(
    (s) => s.username === usuarioLogado.username,
  );
  const temStoryAtivo = meusStories.length > 0;
  const meusStoriesVistos =
    temStoryAtivo &&
    meusStories.every((st) => {
      const vistas = st.visualizacoes || [];
      return vistas.some((v) => v.username === usuarioLogado.username);
    });

  const autoresComStoriesMap = {};
  const storiesOrdenados = [...storiesFiltradosAmigos].sort(
    (a, b) => a.id - b.id,
  );
  ...
  storiesOrdenados.forEach((st) => {
    if (!autoresComStoriesMap[st.username]) {
      const perfilAutor =
        perfisReais.find((p) => p.username === st.username) || {};
  ...
  const listaAutoresStories = Object.values(autoresComStoriesMap).map(
    (autorObj) => {
      const todosVistos = autorObj.lista.every((st) => {
        const visualizacoesDoStory = st.visualizacoes || [];
        return visualizacoesDoStory.some(
          (v) => v.username === usuarioLogado.username,
        );
      });
    ...
    async function computarVisualizacao() {
      if (usuarioStoryVisualizando && storyAtivoObj) {
        ...
        if (!jaViu && storyAtivoObj.username !== usuarioLogado.username) {
          ...
          const storiesAtualizados =
            await BancoDeDados.registrarVisualizacaoStory(
              storyId,
              novaVisualizacao,
            );
  ...
  const avancarStory = () => {
    if (indiceStoryAtual < listaStoriesDoAutorAtual.length - 1) {
      ...
      const currentIndex = listaAutoresStories.findIndex(
        (a) => a.username === usuarioStoryVisualizando,
      );
      if (
        currentIndex !== -1 &&
        currentIndex < listaAutoresStories.length - 1
      ) {
  ...
  const abrirPerfilPorUsername = (usernameAlvo) => {
    ...
    const perfilEncontrado = perfisReais.find(
      (p) => p.username === usernameAlvo,
    );
    if (perfilEncontrado) {
  ...
  const clicarPerfilOuStory = (usernameAlvo) => {
    if (!usernameAlvo) return;
    const autorTemStory = storiesFiltradosAmigos.some(
      (s) => s.username === usernameAlvo,
    );
    if (autorTemStory) {
      setIndiceStoryAtual(0);
  ...
  const enviarMensagemChat = async (e) => {
    ...
    if (typeof BancoDeDados.enviarMensagemChat === "function") {
      ...
      const msgsAtualizadas = await BancoDeDados.getMensagensChat(
        usuarioLogado.username,
        chatComUsuario,
      );
  ...
  const curtirStoryAtual = async () => {
    ...
    const atualizados =
      typeof BancoDeDados.curtirStory === "function"
        ? await BancoDeDados.curtirStory(
            storyAtivoObj.id,
            usuarioLogado.username,
          )
        : [];
  ...
  const compartilharRedesSociais = (rede, post) => {
    const linkUrl = encodeURIComponent(
      `${window.location.origin}${window.location.pathname}#/comunidade?post=${post.id}`,
    );
    const textoUrl = encodeURIComponent(
      `Veja esta publicação de @${post.username}: "${post.tema}" - ${post.texto}`,
    );

    if (rede === "whatsapp") {
      window.open(
        `https://api.whatsapp.com/send?text=${textoUrl}%20-%20${linkUrl}`,
        "_blank",
      );
    } else if (rede === "facebook") {
      window.open(
        `https://www.facebook.com/sharer/sharer.php?u=${linkUrl}`,
        "_blank",
      );
    }
    setMenuCompartilharAberto(null);
  ...
  const salvarEdicaoPost = async (id) => {
    const atualizados = await BancoDeDados.atualizarPublicacao(
      id,
      textoEditado,
      temaEditado,
    );
  ...
  const reagir = async (id, tipoReacao) => {
    const atualizados = await BancoDeDados.reagirPublicacao(
      id,
      tipoReacao,
      usuarioLogado.username,
    );
  ...
  const comentar = async (publicacaoId, usernameAutorPost, e) => {
    ...
    const atualizados = await BancoDeDados.adicionarComentarioPub(
      publicacaoId,
      comentarioObj,
    );
    setPublicacoes([...atualizados]);
    setNovoComentario({ ...novoComentario, [publicacaoId]: "" });
    setRespondendoComentarioId({
    ...
    if (matches) {
      matches.forEach(async (m) => {
        ...
        if (userMencionado !== usuarioLogado.username) {
          await BancoDeDados.adicionarNotificacao(
            userMencionado,
            `@${usuarioLogado.username} mencionou você em um comentário.`,
            "mencao",
          );
        }
      });
    ...
    if (usuarioLogado.username !== usernameAutorPost) {
      await BancoDeDados.adicionarNotificacao(
        usernameAutorPost,
        `@${usuarioLogado.username} comentou na sua publicação.`,
        "comentario",
      );
    }
  };

  const reagirComentarioPub = async (
    publicacaoId,
    comentarioId,
    tipoReacao,
  ) => {
    const atualizados = await BancoDeDados.reagirComentarioPub(
      publicacaoId,
      comentarioId,
      tipoReacao,
      usuarioLogado.username,
  ...
  if (postDetalheId) {
    ...
    return (
      <div
        className={`w-full max-w-2xl mx-auto px-3 sm:px-6 py-6 space-y-6 overflow-x-hidden box-border ${darkMode ? "text-slate-100" : "text-slate-900"}`}
      >
        <button
        ...
        {!postUnico ? (
          <div
            className={`p-8 text-center rounded-3xl border shadow-sm ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
          >
            <p className="text-xs opacity-60">
              Esta publicação não foi encontrada ou foi removida.
            </p>
          </div>
        ) : (
  ...
  const amigosLista = perfisReais.filter((p) =>
    meusAmigos.includes(p.username),
  );
  const outrosUsuarios = perfisReais.filter(
    (p) =>
      p.username !== usuarioLogado.username && !meusAmigos.includes(p.username),
  );
  ...
  const publicacoesFiltradas = publicacoes.filter((post) => {
    ...
    return (
      ...
      (post.autor || "").toLowerCase().includes(termo)
    );
  });
  ...
  const membrosFiltrados = outrosUsuarios.filter((membro) => {
    ...
    return (
      ...
      membro.username.toLowerCase().includes(termo)
    );
  });
  ...
  const perfisSugeridosMencao = perfisReais.filter((p) => {
    ...
    return (
      p.username.toLowerCase().includes(t) || p.nome.toLowerCase().includes(t)
    );
  });
  ...
  function renderizarCardPublicacao(post, isolado = false) {
    ...
    const perfilAutorReal =
      perfisReais.find((p) => p.username === post.username) || {};
    const avatarAtualizado =
      perfilAutorReal.foto ||
      post.avatar ||
      "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80";
    const nomeAtualizado = perfilAutorReal.nome || post.autor;
    const autorVerificado = perfilAutorReal.verificado;
    const autorTemStory = storiesFiltradosAmigos.some(
      (s) => s.username === post.username,
    );
    ...
    return (
      <div
        key={post.id}
        className={`p-4 sm:p-6 rounded-3xl border shadow-md space-y-4 relative w-full overflow-hidden box-border ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
      >
        <div className="flex items-center justify-between">
          ...
          >
            <div
              className={`w-12 h-12 rounded-full p-0.5 flex items-center justify-center flex-shrink-0 transition ${autorTemStory ? "bg-gradient-to-tr from-amber-500 via-rose-600 to-yellow-400 shadow-md animate-pulse" : "border-2 border-blue-500/30"}`}
            >
              <img
                src={avatarAtualizado}
                alt="Avatar"
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1">
                ...
                {autorVerificado && (
                  <span
                    className="relative inline-flex items-center justify-center flex-shrink-0 group/badge cursor-pointer"
                    title="Perfil Verificado"
                  >
                    <svg
                      className="w-5 h-5 text-blue-500 transform transition hover:scale-110"
                      viewBox="0 0 24 24"
                      fill="currentColor"
                    >
                      ...
                      <path
                        d="m9.5 13.79-2.15-2.15a1 1 0 0 0-1.41 1.41l2.86 2.86a1 1 0 0 0 1.41 0l6.14-6.14a1 1 0 0 0-1.41-1.41L9.5 13.79z"
                        fill="#ffffff"
                      />
                    </svg>
                  </span>
          ...
          {souDono && !estaEditando && (
            <div className="relative flex-shrink-0">
              <button
                onClick={() =>
                  setMenuOpcoesPostAberto(
                    menuOpcoesPostAberto === post.id ? null : post.id,
                  )
                }
              ...
              {menuOpcoesPostAberto === post.id && (
                <div
                  className={`absolute right-0 mt-1 w-36 rounded-2xl border shadow-xl py-1 z-30 ${darkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
                >
                  <button
                    onClick={() => {
        ...
        {estaEditando ? (
          <div className="space-y-3 pt-2">
            ...
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setPostEditandoId(null)}
                className="px-3 py-1.5 rounded-xl text-xs opacity-70"
              >
                Cancelar
              </button>
              <button
                onClick={() => salvarEdicaoPost(post.id)}
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed opacity-90 whitespace-pre-line break-words">
              {post.texto}
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <button
              ...
              className={`text-[11px] sm:text-xs px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl font-bold border transition flex items-center gap-1 ${meuAmem ? "bg-blue-600 text-white border-blue-500 shadow-sm" : darkMode ? "bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700" : "bg-white text-slate-800 border-slate-300 hover:bg-slate-50 shadow-xs"}`}
            >
              <svg
            ...
            <button
              ...
              className={`text-[11px] sm:text-xs px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl font-bold border transition flex items-center gap-1 ${meuAmor ? "bg-pink-600 text-white border-pink-500 shadow-sm" : darkMode ? "bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700" : "bg-white text-slate-800 border-slate-300 hover:bg-slate-50 shadow-xs"}`}
            >
              <svg
                className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-pink-500 flex-shrink-0"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                viewBox="0 0 24 24"
          ...
          <div className="relative">
            <button
              onClick={() =>
                setMenuCompartilharAberto(
                  menuCompartilharAberto === post.id ? null : post.id,
                )
              }
            ...
            {menuCompartilharAberto === post.id && (
              <div
                className={`absolute right-0 top-full mt-2 sm:bottom-full sm:mb-2 sm:top-auto w-56 rounded-2xl border shadow-2xl p-2 z-50 space-y-1 ${darkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
              >
                <p className="text-[10px] font-bold uppercase tracking-wider opacity-50 px-2 py-1">
                  Opções de Partilha
                </p>
          ...
          {post.comentarios && post.comentarios.length > 0 && (
            <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800">
              {post.comentarios.map((c) => {
                const perfilAutorComentario =
                  perfisReais.find((p) => p.username === c.username) || {};
                const fotoComentario =
                  perfilAutorComentario.foto ||
                  "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80";
                const autorComentarioVerificado =
                  perfilAutorComentario.verificado;
                const autorComentarioTemStory = storiesFiltradosAmigos.some(
                  (s) => s.username === c.username,
                );

                const reacoesComentario = c.reacoes || {
                  amem: [],
                  aleluia: [],
                  amor: [],
                };
                const meuAmemCom = (reacoesComentario.amem || []).includes(
                  usuarioLogado.username,
                );
                const meuAleluiaCom = (
                  reacoesComentario.aleluia || []
                ).includes(usuarioLogado.username);
                const meuAmorCom = (reacoesComentario.amor || []).includes(
                  usuarioLogado.username,
                );

                const ehResposta = Boolean(c.resposta_a_id);
                const comentarioPai = ehResposta
                  ? post.comentarios.find((cp) => cp.id === c.resposta_a_id)
                  : null;
  ...
  <svg className="w-5 h-5 text-blue-500 transform transition hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
    ...
    <path d="m9.5 13.79-2.15-2.15a1 1 0 0 0-1.41 1.41l2.86 2.86a1 1 0 0 0 1.41 0l6.14-6.14a1 1 0 0 0-1.41-1.41L9.5 13.79z" fill="#ffffff" />
                        ...
                        <p className="opacity-95 break-words leading-relaxed">
                          {c.texto}
                        </p>
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-1">
                          <button
                            onClick={() =>
                              reagirComentarioPub(post.id, c.id, "amem")
                            }
                            className={`text-[10px] font-bold flex items-center gap-1 ${meuAmemCom ? "text-red-500" : "opacity-60 hover:opacity-100"}`}
                          >
                            ❤️ Amém ({(reacoesComentario.amem || []).length})
                          </button>
                          <button
                            onClick={() =>
                              reagirComentarioPub(post.id, c.id, "aleluia")
                            }
                            className={`text-[10px] font-bold flex items-center gap-1 ${meuAleluiaCom ? "text-amber-500" : "opacity-60 hover:opacity-100"}`}
                          >
                            ⭐ Aleluia (
                            {(reacoesComentario.aleluia || []).length})
                          </button>
                          <button
                            onClick={() =>
                              reagirComentarioPub(post.id, c.id, "amor")
                            }
                            className={`text-[10px] font-bold flex items-center gap-1 ${meuAmorCom ? "text-pink-500" : "opacity-60 hover:opacity-100"}`}
                          >
          ...
          <form
            onSubmit={(e) => comentar(post.id, post.username, e)}
          ...
          >
            <input
              ...
              onChange={(e) => {
                ...
                if (
                  ultimoIndiceArroba !== -1 &&
                  (ultimoIndiceArroba === 0 ||
                    val[ultimoIndiceArroba - 1] === " ")
                ) {
                  ...
                  perfisSugeridosMencaoComentario.map((p) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        const textoBase = textoAtual.substring(
                          0,
                          ultimoIndiceArroba,
                        );
                        setNovoComentario({
                          ...novoComentario,
                          [post.id]: `${textoBase}@${p.username} `,
                        });
                        setMenuMencaoComentarioAberto(null);
                      }}
            ...
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-4 py-2.5 rounded-xl font-bold transition shadow-sm flex-shrink-0"
            >
              Enviar
            </button>
          </form>
  ...
  return (
    <div
      className={`w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-10 py-6 space-y-6 overflow-x-hidden box-border ${darkMode ? "text-slate-100" : "text-slate-900"}`}
    >
      {toastMensagem && (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              onClick={async () => {
                ...
                if (!abaNotificacoesAberta) {
                  await BancoDeDados.marcarNotificacoesLidas(
                    usuarioLogado.username,
                  );
                  setNotificacoes(
                    await BancoDeDados.getNotificacoes(usuarioLogado.username),
                  );
                }
              }}
            ...
            {abaNotificacoesAberta && (
              <div
                className={`absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto rounded-3xl border shadow-2xl p-4 z-40 space-y-3 ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
              >
                ...
                {notificacoes.length === 0 ? (
                  ...
                  notificacoes.map((n, idx) => {
                    ...
                    const perfilNotif =
                      perfisReais.find((p) => p.username === usernameNotif) ||
                      {};
                    const fotoNotif =
                      perfilNotif.foto ||
                      "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80";
                    const temStoryNotif = storiesFiltradosAmigos.some(
                      (s) => s.username === usernameNotif,
                    );

                    return (
                      <div
                        key={idx}
                        className={`p-3 rounded-2xl border text-xs flex items-center gap-3 ${darkMode ? "bg-slate-800/50 border-slate-700" : "bg-slate-50 border-slate-200"}`}
                      >
                        {usernameNotif ? (
                          <div
                          ...
                          <div className="w-9 h-9 rounded-full bg-blue-600/20 text-blue-500 flex items-center justify-center font-bold flex-shrink-0">
                            🔔
                          </div>
                        )}
      ...
      {abaSolicitacoesAberta && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div
            className={`max-w-md w-full p-6 rounded-3xl shadow-2xl border space-y-4 ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
          >
            <div className="flex justify-between items-center border-b pb-3 border-slate-700">
              ...
              <button
                onClick={() => setAbaSolicitacoesAberta(false)}
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto">
              {pedidosRecebidos.length === 0 ? (
                ...
                pedidosRecebidos.map((remetenteusername) => {
                  const perfilRemetente = perfisReais.find(
                    (p) => p.username === remetenteusername,
                  ) || { nome: remetenteusername, username: remetenteusername };
                  return (
                    <div
                      key={remetenteusername}
                      className={`p-3 rounded-2xl border flex items-center justify-between text-xs ${darkMode ? "bg-slate-800 border-slate-700" : "bg-slate-50 border-slate-200"}`}
                    >
                      <div className="flex items-center gap-2.5">
                        <img
                          src={
                            perfilRemetente.foto ||
                            "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80"
                          }
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={async () => {
                            ...
                            mostrarToast(
                              `Amizade com @${remetenteusername} aceita! 🎉`,
                            );
                          }}
                        ...
                        <button
                          onClick={async () => {
                            const perfisAtualizados =
                              await BancoDeDados.recusarPedidoAmizade(
                                usuarioLogado.username,
                                remetenteusername,
                              );
                            setPerfisReais(perfisAtualizados);
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => setModalCriarStoryAberto(false)}
            ...
            <div className="grid grid-cols-2 gap-2 bg-slate-800 p-1 rounded-xl">
              <button
                onClick={() => setTipoStoryCriacao("texto")}
                className={`py-2 text-xs font-bold rounded-lg transition ${tipoStoryCriacao === "texto" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              >
                Story de Texto
              </button>
              <button
                onClick={() => setTipoStoryCriacao("midia")}
                className={`py-2 text-xs font-bold rounded-lg transition ${tipoStoryCriacao === "midia" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              >
                Foto / Vídeo / Câmera
              </button>
            </div>
            ...
            {tipoStoryCriacao === "texto" ? (
                      ...
                      {perfisSugeridosMencao.length === 0 ? (
                        ...
                        perfisSugeridosMencao.map((p) => (
                          <div
                            key={p.username}
                            onClick={() => {
                              const ultimoIndiceArroba =
                                textoStory.lastIndexOf("@");
                              const textoBase = textoStory.substring(
                                0,
                                ultimoIndiceArroba,
                              );
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-slate-700 rounded-2xl p-6 text-center space-y-3">
                  {enviandoMidia ? (
                    <p className="text-xs font-bold text-blue-500 animate-pulse py-8">
                      Processando arquivo...
                    </p>
                  ) : midiaStoryUrl ? (
                    tipoMidia === "video" ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-xs opacity-60">
                        Selecione um arquivo ou grave direto da câmera:
                      </p>
                          ...
                          <input
                            ...
                            onChange={async (e) => {
                              ...
                              if (file) {
                                ...
                                try {
                                  if (
                                    typeof BancoDeDados.uploadMidiaStory ===
                                    "function"
                                  ) {
                                    urlPublica =
                                      await BancoDeDados.uploadMidiaStory(file);
                                  }
                                } catch (err) {}
                                if (!urlPublica) {
                                  urlPublica =
                                    await processarArquivoParaUrl(file);
                                }
                                setEnviandoMidia(false);
                                if (urlPublica) {
                                  setTipoMidia(
                ...
                <div className="space-y-1 relative">
                  <label className="text-xs font-bold opacity-70 block">
                    Mencionar amigo (@username):
                  </label>
                  <input
                    type="text"
                  ...
                  {menuSugestoesMencaoAberto && (
                    <div
                      className={`absolute left-0 right-0 bottom-full mb-1 max-h-36 overflow-y-auto rounded-2xl border p-2 space-y-1 shadow-2xl z-20 ${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-300"}`}
                    >
                      {perfisSugeridosMencao.map((p) => (
                        <div
                          key={p.username}
          ...
          <div className="relative max-w-md w-full h-[85vh] bg-slate-900 rounded-3xl overflow-hidden flex flex-col shadow-2xl border border-slate-800">
            ...
            <div className="absolute top-5 left-4 right-4 z-20 flex items-center justify-between">
              <div
                onClick={() => {
                  setUsuarioStoryVisualizando(null);
                  abrirPerfilPorUsername(storyAtivoObj.username);
                }}
                className="flex items-center gap-2 cursor-pointer group"
              >
                <img
                  src={
                    storyAtivoObj.avatar ||
                    "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80"
                  }
                  className="w-9 h-9 rounded-full object-cover border-2 border-amber-500 shadow-md group-hover:scale-105 transition"
                />
                <div>
                  <span className="text-white text-xs font-bold drop-shadow-md block group-hover:underline">
                    {storyAtivoObj.autor}
                  </span>
              ...
              <button
                onClick={() => setUsuarioStoryVisualizando(null)}
                className="bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm hover:bg-black"
              >
                ✕
              </button>
            </div>
            ...
            <div
              onClick={voltarStory}
              className="absolute left-0 top-16 bottom-20 w-1/2 z-20 cursor-pointer"
              title="Anterior"
            ></div>
            <div
              onClick={avancarStory}
              className="absolute right-0 top-16 bottom-20 w-1/2 z-20 cursor-pointer"
              title="Próximo"
            ></div>
            ...
            <div className="flex-1 flex items-center justify-center w-full h-full relative bg-black">
              {storyAtivoObj.tipo === "texto" ? (
                <div
                  className="w-full h-full flex flex-col items-center justify-center p-8 text-center whitespace-pre-line"
                  style={{
                    backgroundColor: storyAtivoObj.cor_fundo || "#1e293b",
                  }}
                >
                  <p className="text-white text-lg sm:text-xl font-extrabold leading-relaxed drop-shadow-md">
                    {storyAtivoObj.conteudo}
                  </p>
                  {storyAtivoObj.mencao && (
                    <span
                      onClick={() => {
                        setUsuarioStoryVisualizando(null);
                        abrirPerfilPorUsername(storyAtivoObj.mencao);
                      }}
            ...
            <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center z-30 gap-2">
              {storyAtivoObj.username === usuarioLogado.username ? (
                <div className="flex items-center justify-between w-full gap-2">
                  ...
                  >
                    👁️ {(storyAtivoObj.visualizacoes || []).length}{" "}
                    Visualizações
                  </button>
                  <button
                    onClick={async () => {
                      await BancoDeDados.excluirStory(storyAtivoObj.id);
                      setStories(await BancoDeDados.getStories());
                      setUsuarioStoryVisualizando(null);
                      mostrarToast("Story excluído.");
                    }}
                    className="bg-red-600 text-white text-xs font-bold px-4 py-2 rounded-xl shadow-lg hover:bg-red-700"
                  >
                    Excluir
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 w-full">
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl shadow-lg flex-1 transition"
                  >
                    ✨ Repostar
                  </button>
                  <button
                    ...
                    className={`p-3 rounded-2xl shadow-lg transition flex items-center justify-center backdrop-blur-sm border ${
                      (storyAtivoObj.curtidas || []).includes(
                        usuarioLogado.username,
                      )
          ...
          <div className="max-w-sm w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl text-white space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                👁️ Quem visualizou este story
              </h3>
              <button
                onClick={() => setModalVisualizadoresAberto(false)}
                className="text-xs opacity-70 font-bold hover:opacity-100"
              >
                ✕
              </button>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto space-y-2.5 pr-1">
              {!storyAtivoObj.visualizacoes ||
              storyAtivoObj.visualizacoes.length === 0 ? (
                ...
                storyAtivoObj.visualizacoes.map((vis, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between bg-slate-800/60 p-2.5 rounded-2xl border border-slate-700/50"
                  >
                    <div
                    ...
                    >
                      ...
                      />
                      <div className="min-w-0">
                        <p className="text-xs font-bold truncate text-white hover:underline">
                          {vis.nome}
                        </p>
      ...
      <div
        className={`max-w-4xl mx-auto p-4 rounded-2xl border shadow-sm flex items-center gap-3 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
      >
        <svg
        ...
        {termoBuscaComunidade && (
          <button
            onClick={() => setTermoBuscaComunidade("")}
            className="text-xs opacity-60 hover:opacity-100 font-bold px-2"
          >
            Limpar
          </button>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-3 space-y-6">
          <div
            className={`p-6 rounded-3xl border shadow-md space-y-4 text-center ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
          >
            <div
              onClick={() => abrirPerfilPorUsername(usuarioLogado.username)}
            ...
            >
              <div
                className={`w-24 h-24 rounded-full p-1 mx-auto flex items-center justify-center transition ${temStoryAtivo ? (meusStoriesVistos ? "border-2 border-slate-500/40 opacity-75" : "bg-gradient-to-tr from-amber-500 via-rose-600 to-yellow-400 animate-pulse shadow-xl") : ""}`}
              >
                <img
                  src={fotoPerfilOficial}
                  alt="Avatar"
                  className="w-full h-full rounded-full object-cover border-2 border-white dark:border-slate-900 shadow-md group-hover:opacity-90 transition"
                />
              </div>
              {temStoryAtivo && (
                <span
                  className={`absolute bottom-0 right-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded-full shadow-md ${meusStoriesVistos ? "bg-slate-600 text-slate-300" : "bg-gradient-to-r from-rose-600 to-amber-500 text-white"}`}
                >
                  Story
                </span>
              )}
            ...
            <div>
              <div className="flex items-center justify-center gap-1">
                <h3
                  onClick={() => abrirPerfilPorUsername(usuarioLogado.username)}
                  className="font-extrabold text-sm cursor-pointer hover:text-blue-500 hover:underline transition"
                >
                  {nomePerfilOficial}
                </h3>
              ...
              <p
                onClick={() => abrirPerfilPorUsername(usuarioLogado.username)}
                className="text-xs text-blue-500 font-bold mt-0.5 cursor-pointer hover:underline"
              >
                @{usuarioLogado.username}
              </p>
              <p className="text-xs opacity-75 mt-2">
                {meuPerfilBanco.biografia ||
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-2 text-center">
              <div
                className={`p-3 rounded-2xl border shadow-xs ${darkMode ? "bg-slate-800/40 border-slate-700" : "bg-white border-slate-200"}`}
              >
              ...
              <div
                className={`p-3 rounded-2xl border shadow-xs ${darkMode ? "bg-slate-800/40 border-slate-700" : "bg-white border-slate-200"}`}
              >
                <span className="block font-extrabold text-indigo-500 text-sm">
                  {
                    publicacoes.filter(
                      (p) => p.username === usuarioLogado.username,
                    ).length
        ...
        <div className="lg:col-span-6 space-y-6">
          <div
            className={`p-4 rounded-3xl border shadow-md flex gap-3 overflow-x-auto ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
          >
            <div
            ...
            >
              <div
                ...
                style={{ backgroundImage: `url(${fotoPerfilOficial})` }}
              ></div>
              <div className="absolute top-3 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md">
                +
              </div>
              <span className="relative z-10 text-[11px] font-bold text-center px-1">
                Adicionar story
              </span>
            </div>
            ...
            {listaAutoresStories.map((autorItem) => {
              ...
              return (
                ...
                >
                  {st.tipo === "texto" ? (
                    <div
                      className="absolute inset-0 p-3 flex items-center justify-center text-center"
                      style={{ backgroundColor: st.cor_fundo || "#1e293b" }}
                    >
                      <p className="text-white text-[11px] font-bold line-clamp-4">
                        {st.conteudo}
                      </p>
                    </div>
                  ) : st.tipo === "video" ? (
                  ...
                  <div
                    className={`relative z-10 w-8 h-8 rounded-full p-0.5 shadow-md ${todosVistos ? "border border-slate-400 bg-slate-600" : "bg-gradient-to-tr from-amber-500 via-rose-600 to-yellow-400"}`}
                  >
                    <img
                      src={autorItem.avatar}
          ...
          <div
            className={`p-6 rounded-3xl border shadow-md space-y-4 ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
          >
            ...
            <form onSubmit={publicarPost} className="space-y-3">
              <input
                type="text"
                placeholder="Tema da publicação..."
                value={pubTema}
                onChange={(e) => setPubTema(e.target.value)}
                className={`w-full text-sm rounded-xl px-4 py-2.5 border font-bold ${darkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-50 border-slate-300"}`}
              />
              <textarea
                rows="3"
                ...
                className={`w-full text-sm rounded-xl px-4 py-2.5 border ${darkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-50 border-slate-300"}`}
              ></textarea>
              {pubImagem && (
                <img
                  src={pubImagem}
              ...
              <div className="flex justify-between items-center">
                <label
                  className={`text-xs px-4 py-2 rounded-xl cursor-pointer flex items-center gap-1.5 transition font-semibold ${darkMode ? "bg-slate-800 hover:bg-slate-700 text-slate-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}
                >
                  📷 Imagem
                  <input
                    type="file"
                    ...
                    onChange={async (e) => {
                      ...
                      if (f) {
                        const url = await processarArquivoParaUrl(f);
                        setPubImagem(url);
                      }
                    }}
                    className="hidden"
                ...
                <button
                  type="submit"
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition shadow-md"
                >
                  Publicar
                </button>
              </div>
          ...
          <div
            className={`p-6 rounded-3xl border shadow-md space-y-3 ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
          >
            <h3 className="text-xs font-bold uppercase tracking-wider opacity-60">
            ...
            <form onSubmit={criarPedidoOracaoHandler} className="flex gap-2">
              ...
              <button
                type="submit"
                className="bg-blue-600 text-white text-xs px-5 py-2.5 rounded-xl font-bold transition hover:bg-blue-700 shadow-sm flex-shrink-0"
              >
                Pedir Oração
              </button>
            </form>
              ...
              {pedidosOracao.length === 0 ? (
                ...
                pedidosOracao.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`p-3.5 rounded-2xl border flex items-center justify-between text-xs gap-2 ${darkMode ? "bg-slate-800/40 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-800"}`}
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          onClick={async () => {
                            const atualizados =
                              typeof BancoDeDados.apoiarPedidoOracao ===
                              "function"
                                ? await BancoDeDados.apoiarPedidoOracao(
                                    p.id,
                                    usuarioLogado.username,
                                  )
                                : [];
                          ...
                          <button
                            onClick={async () => {
                              if (window.confirm("Excluir pedido?")) {
                                const atualizados =
                                  await BancoDeDados.excluirPedidoOracao(p.id);
                                setPedidosOracao(atualizados || []);
                                mostrarToast("Pedido excluído.");
                              }
                            }}
                            className="text-slate-400 hover:text-red-500 p-1.5 font-bold transition"
                          >
                            ✕
                          </button>
                        )}
          ...
          <div className="space-y-6">
            ...
            {publicacoesFiltradas.length === 0 ? (
              ...
              publicacoesFiltradas.map((post) =>
                renderizarCardPublicacao(post, false),
              )
            )}
          </div>
        </div>
            ...
            <div className="space-y-3">
              ...
              {amigosLista.length === 0 ? (
                <p className="text-xs opacity-40 text-center py-6">
                  Nenhum amigo conectado no chat ainda.
                </p>
              ) : (
                amigosLista.map((amigo) => {
                  const naoLidasDoAmigo = notificacoes.filter(
                    (n) =>
                      !n.lida &&
                      n.tipo === "mensagem" &&
                      n.texto.includes(`@${amigo.username}`),
                  ).length;
                  const amigoTemStory = storiesFiltradosAmigos.some(
                    (s) => s.username === amigo.username,
                  );
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1 min-w-0">
                            <p
                              onClick={(e) => {
                                e.stopPropagation();
                                abrirPerfilPorUsername(amigo.username);
                              }}
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
              {membrosFiltrados.length === 0 ? (
                ...
                membrosFiltrados.map((membro) => {
                  const enviei = meuPerfilBanco.pedidos_enviados?.includes(
                    membro.username,
                  );
                  const membroTemStory = storiesFiltradosAmigos.some(
                    (s) => s.username === membro.username,
                  );

                  return (
                    <div
                      key={membro.username}
                      className={`p-3 rounded-2xl border flex items-center justify-between text-xs gap-2 ${darkMode ? "bg-slate-800/30 border-slate-700" : "bg-slate-50 border-slate-200"}`}
                    >
                      <div
                        className="flex items-center gap-2.5 min-w-0 cursor-pointer"
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full p-0.5 flex items-center justify-center flex-shrink-0 transition ${membroTemStory ? "bg-gradient-to-tr from-amber-500 via-rose-600 to-yellow-400 shadow-sm animate-pulse" : ""}`}
                        >
                          <img
                            src={
                              membro.foto ||
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1 min-w-0">
                            <p
                              onClick={(e) => {
                                e.stopPropagation();
                                abrirPerfilPorUsername(membro.username);
                              }}
  ...
  <svg className="w-5 h-5 text-blue-500 transform transition hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
    ...
    <path d="m9.5 13.79-2.15-2.15a1 1 0 0 0-1.41 1.41l2.86 2.86a1 1 0 0 0 1.41 0l6.14-6.14a1 1 0 0 0-1.41-1.41L9.5 13.79z" fill="#ffffff" />
                      ...
                      <button
                        disabled={enviei}
                        title={
                          enviei ? "Solicitação Pendente" : "Seguir / Adicionar"
                        }
                        onClick={async () => {
                          ...
                          mostrarToast(
                            `Pedido de amizade enviado para @${membro.username}!`,
                          );
                        }}
      ...
      {chatComUsuario ? (
        <div className="fixed bottom-4 right-4 z-50 w-[360px] sm:w-[380px] h-[500px] max-h-[85vh] rounded-3xl shadow-2xl border flex flex-col overflow-hidden backdrop-blur-md bg-slate-900 border-slate-700 animate-in fade-in zoom-in-95 duration-200">
            ...
            {mensagensChat.length === 0 ? (
              ...
              mensagensChat.map((m, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex flex-col max-w-[80%] ${souEu ? "ml-auto items-end" : "mr-auto items-start"}`}
                  >
                    <div
                      className={`p-3 rounded-2xl text-xs shadow-sm ${souEu ? "bg-blue-600 text-white rounded-br-none" : "bg-slate-800 text-slate-200 rounded-bl-none"}`}
                    >
                      {m.midia ? (
                        m.tipoMidia === "video" ? (
                          <video
                            src={m.midia}
                            controls
                            className="w-48 h-36 object-cover rounded-xl mb-1"
                          />
                        ) : (
                          <img
                            src={m.midia}
          ...
          >
            ...
            <label
              className="text-slate-400 hover:text-white p-1.5 cursor-pointer transition text-base"
              title="Enviar Imagem"
            >
              📷
              <input
                type="file"
                accept="image/*"
                onChange={(e) => enviarMidiaChat(e, "imagem")}
                className="hidden"
              />
            </label>
            ...
            >
              ...
              <input
                type="file"
                accept="video/*"
                onChange={(e) => enviarMidiaChat(e, "video")}
                className="hidden"
              />
            </label>
            ...
            <input
              type="text"
              placeholder={
                enviandoMidia ? "Processando..." : "Digite sua mensagem..."
              }
            ...
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2 rounded-xl font-bold transition shadow-sm"
            >
              Enviar
            </button>
          </form>
          ...
          <button
            onClick={() => {
              if (amigosLista.length > 0) {
                ...
                mostrarToast(
                  "Você precisa ter amigos adicionados na comunidade para iniciar um chat rápido!",
                );
              }
            }}
