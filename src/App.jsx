...
export default function App() {
      ...
      if (session && session.user) {
        ...
        const perfilEncontrado = perfis.find(
          (p) =>
            ...
            p.username === session.user.user_metadata?.username,
        );
        ...
        if (perfilEncontrado) {
          ...
          const novoPerfil = {
            ...
            foto:
              ...
              session.user.user_metadata?.foto ||
  ...
  useEffect(() => {
    async function carregarTemaDoBanco() {
      if (usuarioLogado && usuarioLogado.username) {
        try {
          const perfis = await BancoDeDados.getPerfisCadastrados();
          const meuPerfilBanco = perfis?.find(
            (p) => p.username === usuarioLogado.username,
          );
    ...
    if (usuarioLogado && usuarioLogado.username) {
      try {
        if (typeof BancoDeDados.atualizarTemaUsuario === "function") {
          await BancoDeDados.atualizarTemaUsuario(
  ...
  const initialPath = decodeURIComponent(
  ...
  const [abaPrincipal, setAbaPrincipal] = useState(
    isSystemRoute ? initialPath || "biblia" : "perfilUrl",
  );
  ...
  const versiculosDoDia = [
    ...
    {
      texto: "O Senhor é o meu pastor; de nada faltará.",
    ...
    {
      texto:
        "Porque sou eu que conheço os planos que tenho para vocês, diz o Senhor, planos de fazê-los prosperar e não de causar dano.",
  ...
  useEffect(() => {
    ...
    async function carregarDadosCabecalho() {
      ...
      const rels = await BancoDeDados.getRelacoesAmizade(
        usuarioLogado.username,
      );
      ...
      const pedidosPendentesUser = rels
        .filter(
          (r) =>
            r.amigo_id === usuarioLogado.username && r.status === "pendente",
        )
        .map((r) => r.usuario_id);
      ...
      const amigosAceitos = rels
        ...
        .map((r) =>
          r.usuario_id === usuarioLogado.username ? r.amigo_id : r.usuario_id,
        );

      const meusEnviosPendentes = rels
        .filter(
          (r) =>
            r.usuario_id === usuarioLogado.username && r.status === "pendente",
        )
        .map((r) => r.amigo_id);

      const sugestoes = perfis.filter(
        (p) =>
          p.username !== usuarioLogado.username &&
    ...
    const tratarRotaUrl = async () => {
      ...
      if (!path || path === "biblia") {
        ...
        const encontrado = perfis?.find(
          (p) => p.username?.toLowerCase() === path.toLowerCase(),
        );
  ...
  useEffect(() => {
    setCarregando(true);
    fetch(
      `https://raw.githubusercontent.com/thiagobodruk/biblia/master/json/${versaoSelecionada}.json`,
    )
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "notas_versiculos_biblia",
      JSON.stringify(notasPessoais),
    );
  ...
  const totalCapitulosDoLivro = livroAtualObj.chapters
    ? livroAtualObj.chapters.length
    : 1;
  const versiculosDoCapitulo =
    livroAtualObj.chapters && livroAtualObj.chapters[capituloAtual - 1]
      ? livroAtualObj.chapters[capituloAtual - 1]
      : [];

  return (
    <div
      className={`flex flex-col min-h-screen font-sans ${darkMode ? "bg-slate-950 text-slate-100" : "bg-slate-100 text-slate-800"}`}
    >
      {/* HEADER SUPERIOR */}
      <header
        className={`border-b px-4 lg:px-8 py-3 flex items-center justify-between gap-3 shadow-sm backdrop-blur-md z-40 sticky top-0 ${darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
      >
        ...
        <div className="flex items-center gap-3">
          <div className="relative lg:hidden" ref={hamburguerRef}>
            ...
            {menuHamburguerAberto && (
              <div
                className={`absolute left-0 mt-3 w-64 rounded-2xl shadow-2xl border p-3 z-50 space-y-3 backdrop-blur-md ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
              >
                {abaPrincipal === "biblia" && (
                  <div className="pb-2 border-b border-slate-700/50">
                    <p className="text-[10px] uppercase tracking-wider font-extrabold opacity-60 mb-1">
                      Versão da Bíblia
                    </p>
                    <select
                ...
                <div className="space-y-1">
                  <p className="text-[10px] uppercase tracking-wider font-extrabold opacity-60 px-2 mb-1">
                    Navegação
                  </p>
        ...
        <div
          className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border w-40 lg:w-60 ${darkMode ? "bg-slate-800 border-slate-700" : "bg-slate-100 border-slate-300"}`}
        >
          <svg
        ...
        <div className="flex items-center gap-3">
          ...
          {usuarioLogado && (
            <div className="relative" ref={amigosRef}>
              ...
              {menuAmigosAberto && (
                <div
                  className={`absolute right-0 mt-3 w-80 sm:w-96 rounded-2xl shadow-2xl border p-4 z-50 space-y-4 backdrop-blur-md max-h-96 overflow-y-auto ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
                >
                  ...
                  <div>
                    ...
                    {solicitacoesPendentes.length === 0 ? (
                      <p className="text-xs opacity-60 py-2">
                        Nenhum pedido de amizade no momento.
                      </p>
                    ) : (
                      <div className="space-y-2">
                        {solicitacoesPendentes.map((remetenteUsername) => {
                          const perfilRemetente = perfisCache.find(
                            (p) => p.username === remetenteUsername,
                          );
                          const nomeRemetente =
                            perfilRemetente?.nome || remetenteUsername;
                          const fotoRemetente =
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex gap-1.5 flex-shrink-0">
                                ...
                                <button
                                  onClick={async () => {
                                    await BancoDeDados.removerAmizadeOuPedido(
                                      usuarioLogado.username,
                                      remetenteUsername,
                                    );
                                    setSolicitacoesPendentes((prev) =>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {sugestoesMembros.length === 0 ? (
                        ...
                        sugestoesMembros.map((membro) => (
                          <div
                            key={membro.username}
                            className="flex items-center justify-between p-2 rounded-xl hover:bg-slate-800/50"
                          >
                            <div
                              onClick={() => {
                                setMenuAmigosAberto(false);
                                navegarPara(`/${membro.username}`, "perfilUrl");
                              }}
                            ...
                            <button
                              onClick={async () => {
                                ...
                                setSugestoesMembros((prev) =>
                                  prev.filter(
                                    (u) => u.username !== membro.username,
                                  ),
                                );
                              }}
          ...
          {usuarioLogado && (
            <div className="relative" ref={notificacoesRef}>
                ...
                onClick={async () => {
                  ...
                  if (novoEstado) {
                    ...
                    const notifsFormatadas = (notifs || []).map((n) => {
                      ...
                      const usernameExtraido = matchArroba
                        ? matchArroba[1]
                        : null;
                      const perfilEncontrado = perfis?.find(
                        (p) => p.username === usernameExtraido,
                      );
                      ...
                      const dataIso =
                        ...
                        n.criado_em ||
                        n.createdAt ||
                        (!isNaN(timestampDoId) ? timestampDoId : null);
                      ...
                      return {
                        ...
                        avatarRemetente:
                          ...
                          "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80",
                        nomeRemetente:
                          perfilEncontrado?.nome || usernameExtraido,
                      };
                    });
              ...
              {menuNotificacoesAberto && (
                <div
                  className={`absolute right-0 mt-3 w-80 sm:w-96 rounded-2xl shadow-2xl border p-3 z-50 space-y-2 backdrop-blur-md max-h-96 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:transparent [&::-webkit-scrollbar-thumb]:bg-slate-700 [&::-webkit-scrollbar-thumb]:rounded-full ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
                >
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {menuPerfilAberto && usuarioLogado && (
              <div
                className={`absolute right-0 mt-3 w-56 rounded-2xl shadow-2xl border p-2 z-50 space-y-1 backdrop-blur-md ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
              >
                ...
                <button
                  onClick={() =>
                    navegarPara(`/${usuarioLogado.username}`, "perfilUrl")
                  }
            ...
            {abaPrincipal === "biblia" &&
              (carregando ? (
                ...
                <div className="space-y-3">
                  <h3 className="text-xs font-bold opacity-70 mb-3">
                    Resultados para: "{termoBusca}" ({resultadosBusca.length})
                  </h3>
                  {resultadosBusca.map((res, i) => (
                    <div
                      key={i}
                      className={`p-4 rounded-2xl border ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200 shadow-xs"}`}
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[11px] font-bold text-blue-500">
                          {res.livroNome} {res.capitulo}:{res.numero}
                        </span>
                        <button
                          onClick={() => {
                            const idx = bibliaCompleta.findIndex(
                              (l) => l.name === res.livroNome,
                            );
                            if (idx !== -1) {
                              setLivroIndex(idx);
                ...
                <div className="space-y-6">
                  <div
                    className={`p-5 rounded-3xl border shadow-xs ${darkMode ? "bg-slate-900/80 border-slate-800 text-blue-200" : "bg-blue-50/70 border-blue-100 text-blue-900"}`}
                  >
                  ...
                  <div
                    className={`flex flex-wrap gap-3 items-center justify-between p-4 rounded-2xl border ${darkMode ? "bg-slate-900/50 border-slate-800/80" : "bg-white border-slate-200 shadow-2xs"}`}
                  >
                    <select
                      value={livroIndex}
                      onChange={(e) => {
                  ...
                  <div className={`p-6 sm:p-10 rounded-3xl border shadow-sm relative ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900'}`}>
  <p className="text-xl sm:text-2xl leading-[2.5] tracking-[0.015em] text-justify font-normal">
    {versiculosDoCapitulo.map((textoVersiculo, index) => {
      ...
      // Mapeamento de cores vivas e translúcidas estilo YouVersion
      ...
      return (
        ...
        >
          <sup className="text-xs sm:text-sm font-bold mr-1.5 opacity-60 select-none align-super">
            {numeroV}
          </sup>
          ...
          {isFavorito && <span className="text-xs select-none">❤️</span>}
                    ...
                    {versiculosSelecionados.length > 0 && (
                      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-sm rounded-3xl shadow-2xl border p-4 backdrop-blur-md bg-slate-900 border-slate-700 text-white animate-in fade-in zoom-in-95 duration-200 space-y-4">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {[
                              ...
                              },
                              {
                                nome: "Azul",
                                cor: "bg-sky-400",
                                tailwind: "bg-sky-300/60 dark:bg-sky-400/40",
                              },
                              {
                                nome: "Laranja",
                                cor: "bg-orange-400",
                              ...
                              },
                              {
                                nome: "Rosa",
                                cor: "bg-rose-400",
                                tailwind: "bg-rose-300/60 dark:bg-rose-400/40",
                              },
                            ].map((c) => (
                              <button
                                key={c.nome}
                        ...
                        <button
                          onClick={() => {
                            const textoCompleto = versiculosSelecionados
                              .map((v) => v.texto)
                              .join(" ");
                        ...
                        <button
                          onClick={() => {
                            alert(
                              "Funcionalidade de comparar traduções em breve!",
                            );
                          }}
            ...
            {abaPrincipal === "comunidade" && usuarioLogado && (
              <Comunidade
                usuarioLogado={usuarioLogado}
                darkMode={darkMode}
                onVerPerfil={(username) =>
                  navegarPara(`/${username}`, "perfilUrl")
                }
                abaAtual={abaPrincipal}
                setAbaAtual={(novaAba) =>
                  navegarPara(
                    novaAba === "biblia" ? "/" : `/${novaAba}`,
                    novaAba,
                  )
                }
              />
            ...
            {abaPrincipal === "perfilUrl" && (
              <PerfilPublico
                ...
                onVoltar={() =>
                  navegarPara(
                    usuarioLogado ? "/comunidade" : "/",
                    usuarioLogado ? "comunidade" : "biblia",
                  )
                }
                darkMode={darkMode}
      ...
      <footer
        className={`w-full py-8 px-4 sm:px-8 border-t mt-auto transition-colors duration-200 ${
          darkMode
        ...
        <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="space-y-1">
            <p className="text-xs font-medium">
              Luz do Mundo &copy; {new Date().getFullYear()} — Todos os direitos
              reservados.
            </p>
