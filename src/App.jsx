            ...
            {abaPrincipal === 'biblia' && (
              carregando ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-3">
                  ...
                  <p className="text-xs font-semibold opacity-60">Carregando sagradas escrituras...</p>
                </div>
                ...
                <div className="space-y-6 max-w-3xl mx-auto">
                  ...
                  {/* PALAVRA DO DIA */}
                  <div className={`p-5 sm:p-6 rounded-2xl border relative overflow-hidden transition ${
                    darkMode 
                      ? 'bg-gradient-to-br from-slate-900 via-slate-900 to-blue-950/40 border-slate-800 text-blue-100' 
                      : 'bg-gradient-to-br from-blue-50/80 via-white to-amber-50/50 border-blue-100 text-blue-950 shadow-2xs'
                  }`}>
                  ...
                  <div className={`flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl border ${
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-[200px]">
                      <select
                        ...
                        className={`text-xs font-bold rounded-xl px-3 py-2 border cursor-pointer w-full focus:outline-none transition ${
                      ...
                      <select
                        ...
                        className={`text-xs font-bold rounded-xl px-3 py-2 border cursor-pointer focus:outline-none transition ${
                          darkMode ? 'bg-slate-800 border-slate-700 text-blue-400' : 'bg-slate-50 border-slate-300 text-blue-600'
                        }`}
                      >
                        {Array.from({ length: totalCapitulosDoLivro }, (_, i) => i + 1).map((numCap) => (
                          <option key={numCap} value={numCap}>Cap. {numCap}</option>
                        ))}
                      </select>
                    </div>
                      ...
                      <button
                        onClick={() => {
                          if (capituloAtual > 1) {
                            ...
                            const novoLivroIdx = livroIndex - 1;
                            const prevLivro = bibliaCompleta[novoLivroIdx];
                            setLivroIndex(novoLivroIdx);
                            setCapituloAtual(prevLivro.chapters.length);
                            setVersiculosSelecionados([]);
                            window.scrollTo({ top: 0, behavior: 'smooth' });
                          }
                        }}
                      ...
                      <button
                        onClick={() => {
                          if (capituloAtual < totalCapitulosDoLivro) {
                            ...
                            setVersiculosSelecionados([]);
                            window.scrollTo({ top: 0, behavior: 'smooth' });
                          } else if (livroIndex < bibliaCompleta.length - 1) {
